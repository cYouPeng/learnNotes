	...
	public static void main(String[] args){
		try {
			...
			while(true){
				...
				String requestRow = arr[0];
				String[] rows = requestRow.split(" ");
				String requestData = "";
				if(rows[0].equalsIgnoreCase("GET")){
					requestData = rows[1].substring(rows[1].indexOf("?")+1);
				}
				if(rows[0].equalsIgnoreCase("POST")){
...
//				Content-type:text/html; charset=utf-8
				...
				if(dataMap.get("id")!=null&&dataMap.get("id").equals("abc")&&dataMap.get("pwd")!=null&&dataMap.get("pwd").equals("123")){
